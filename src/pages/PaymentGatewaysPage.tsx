...
export default function PaymentGatewaysPage() {
  ...
  useEffect(() => {
    ...
    tenantRepository.getPaymentConfig(ctx).then((c) => {
      const gateway = c.gateway === 'mercadopago' || c.gateway === 'stripe' ? c.gateway : undefined
  ...
  const handleSelect = (id: PaymentGatewayId) => {
    ...
    toast(
      next ? `${name} conectado — ya puedes pegar credenciales y generar links` : 'Pasarela desconectada',
      'success',
    )
  }
  ...
  const handleSaveCredentials = async () => {
    if (!ctx || !config.gateway) {
      toast('Selecciona Mercado Pago o Stripe primero', 'error')
      return
    }
    setSaving(true)
    try {
      await tenantRepository.updatePaymentConfig(ctx, config)
      toast('Credenciales guardadas — genera links desde POS → Tarjeta', 'success')
    } catch (e) {
    ...
    <div className="max-w-5xl space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">
          Conecta Mercado Pago o Stripe — IA·RESTAURANT solo te ayuda a generar links de pago
        </p>
      </div>
      ...
      <div className="rounded-2xl border-2 border-brand-200 bg-gradient-to-r from-brand-50 to-orange-50 p-5 space-y-4">
        ...
        <div className="flex items-start gap-3 pt-2 border-t border-brand-200/80">
          ...
          <p className="text-xs text-slate-600 leading-relaxed">
            <strong>Suscripción del software:</strong> {SAAS_BILLING_NOTE}
          </p>
        </div>
      </div>

      <div className="rounded-2xl border border-command-border bg-white p-5">
        <p className="font-bold text-slate-800 mb-4">Cómo conectar tu cuenta (3 pasos)</p>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Se guardan en tu organización. IA·RESTAURANT las usa solo para crear links — el cobro lo hace {activeGateway.name}.
              </p>
            </div>
          </div>
      ...
      <p className="text-center text-[10px] text-slate-400 font-mono pb-4 leading-relaxed max-w-lg mx-auto">
        En POS → método Tarjeta → “Generar link de pago”. El comensal paga en {activeGateway?.name || 'MP o Stripe'};
        confirma el cobro en caja cuando veas el pago en tu panel del proveedor.
      </p>
    </div>
  )
}
